...
This guide integrates enterprise patterns with community-standard best practices from the [PowerShell Practice and Style Guide](https://poshcode.gitbook.io/powershell-practice-and-style) maintained by the PoshCode community.
...
- [Function Structure & Cmdlet Design](#function-structure--cmdlet-design)
- [Output Streams & Formatting](#output-streams--formatting)
- [Readability & Commenting Standards](#readability--commenting-standards)
...
Consistency in code formatting makes scripts easier to read, understand, and maintain across teams. While project-specific rules may vary, following these community-standard conventions helps create familiar, professional code.

**Key Principle**: If you're contributing to an existing project, always follow that project's established style guidelines. Consistency within a codebase is more important than personal preference.
...
PowerShell uses **PascalCase** for all public identifiers:
- Module names
- Function/cmdlet names (`Get-User`, `Test-Connection`)
- Class, enum, and attribute names
- Public fields or properties
- Global variables and constants
- **Parameters** (even though they're properties of .NET classes)

**PowerShell keywords** are written in **lowercase**: `foreach`, `if`, `function`, `param`, `begin`, `process`, `end`

**Comment-based help keywords** are **UPPERCASE**: `.SYNOPSIS`, `.DESCRIPTION`, `.PARAMETER`, `.EXAMPLE`

**Special case for acronyms**: Two-letter acronyms have both letters capitalized (e.g., `$PSBoundParameters`, `Get-PSDrive`). Three+ letter acronyms follow PascalCase (e.g., `Start-AzureRmVM`).
...
**Private variables** may use camelCase to distinguish them from parameters (optional):
```powershell
function Get-UserData {
    param([string]$UserName)  # PascalCase parameter
    
    $userAccount = Get-ADUser $UserName  # camelCase private variable
    $adComputer = Get-ADComputer "PC01"  # camelCase with two-letter acronym
}
```

### One True Brace Style (OTBS)

This guide recommends the **One True Brace Style** variant:
- **Opening brace** on the **end of a line**
- **Closing brace** at the **beginning of a line**
...
# Exception: Single-line scriptblocks for pipeline
Get-ChildItem | Where-Object { $_.Length -gt 10MB }
```

**Why OTBS?**
1. No exceptions needed - works consistently everywhere
2. Safe to insert new lines anywhere without breaking code
3. Required for DSC and scriptblock parameters (`ForEach-Object`, `Where-Object`)
4. Historical PowerShell console compatibility
5. Reduces errors from misplaced braces
...
**Continuation lines** may indent more than 4 spaces to align with method calls or parameters:
```powershell
function Test-Code {
    $result = [System.Math]::Pow($base,
                                  $exponent)  # Aligned with opening parenthesis
    
    $longVariable = Get-Process |
...
Reasons:
- PowerShell console is 120 characters wide by default
- Line continuation prompt `>>>` uses 116 characters max
- GitHub displays 115-126 characters depending on browser
- Allows side-by-side diffs without scrolling on 1080p monitors

**Preferred line breaking techniques** (in order of preference):
1. **Splatting** for cmdlet parameters
2. **Implicit line continuation** inside parentheses, brackets, braces
3. **String concatenation** with `+` operator
4. **Backtick** `` ` `` only as last resort
...
$message = (
    ...
    "Using parentheses allows clean line breaks."
)

# Avoid: Backtick for line continuation
$result = Get-Process `
    -Name powershell `
    -ErrorAction SilentlyContinue  # Fragile, easy to break
```

### Blank Lines and Whitespace

**Surround function and class definitions with 2 blank lines:**
```powershell
function Get-User { }
...
**Method definitions within a class use 1 blank line:**
```powershell
class UserAccount {
    [string]$Name
...
**Remove trailing whitespace** - trailing spaces cause unnecessary diffs in version control.

### Spaces Around Operators and Parameters

**Use single space around operators and parameter names:**
```powershell
# Correct
...
**Exception for switch parameters** - no space around colon:
```powershell
-Wait:$true
-Recurse:($Depth -gt 1)
```

**Exception for unary operators** - no space:
```powershell
...
```powershell
# Good
$result = $( Get-ChildItem | Measure-Object )
$filtered = Get-Process | Where-Object { $_.CPU -gt 100 }

# Variable delimiter - no spaces
"This is ${VariableName} in a string"
```
...
PowerShell doesn't require semicolons to end statements. **Avoid using them** except when necessary (multiple statements on one line).
...
- [ ] Use PascalCase for functions, parameters, and public identifiers
- [ ] Use lowercase for PowerShell keywords
- [ ] Follow One True Brace Style (opening brace on same line)
- [ ] Indent with 4 spaces (no tabs)
- [ ] Limit lines to 115 characters
- [ ] Single space around operators and parameters
- [ ] No trailing whitespace
- [ ] Avoid semicolons as line terminators
- [ ] 2 blank lines between functions
- [ ] End file with single blank line

---

## Naming Conventions

### Cmdlet Naming: Verb-Noun Pattern

All PowerShell functions and cmdlets must follow the **Verb-Noun** naming convention using **approved verbs**.

**Get approved verbs:**
```powershell
Get-Verb  # Lists all approved verbs with descriptions
```
...
- **Start/Stop** - Begin/end operation
- **Enable/Disable** - Turn on/off
- **Add/Remove** - Add to or remove from collection
- **Invoke** - Execute action
- **Update** - Refresh or modernize

```powershell
# Correct - Approved verbs
function Get-UserAccount { }
function Set-UserProperty { }
function Test-NetworkConnection { }
function New-ServiceAccount { }

# Incorrect - Non-approved verbs
function Retrieve-UserAccount { }   # Use Get-
function Create-ServiceAccount { }  # Use New-
function Fetch-Data { }             # Use Get-
```

**Noun should be singular** (even when operating on multiple items):
```powershell
# Correct
Get-User
Set-Mailbox
...
**Use PowerShell standard parameter names** for consistency with built-in cmdlets:

```powershell
# Preferred standard names
$ComputerName    # Not: $Computer, $Server, $Machine
$ServerInstance  # For SQL Server instances
$Path           # Not: $FilePath, $FileName
$Name           # Generic name property
$Identity       # For user/group identifiers
$Credential     # For PSCredential objects
$Force          # To bypass confirmations
$WhatIf         # Preview mode (via [CmdletBinding(SupportsShouldProcess)])
...
function Set-UserMailbox {
    ...
    param(
        ...
        [Parameter(Mandatory)]
        [string]$Identity,              # User identifier
        
        [string]$DisplayName,           # Not: $Name, $FullName
        
        [string[]]$EmailAddresses,      # Plural for arrays
        
        [string]$PrimarySmtpAddress,    # Specific, descriptive
        
        [ValidateSet('User','Shared','Room')]
        [string]$MailboxType,           # Not: $Type (too generic)
        
        [PSCredential]$Credential,      # Standard credential parameter
        
        [switch]$Force                  # Standard switch for bypass
    )
}
```
...
**Script-scope variables** use `$Script:` prefix:
```powershell
$Script:Results = @()
$Script:ConnectionStatus = $false
```

**Global variables** use `$Global:` prefix (use sparingly):
```powershell
$Global:ConfigurationPath = "C:\Config"
```

**Private variables** may use camelCase (optional):
```powershell
function Get-UserData {
    param([string]$UserName)       # PascalCase parameter
    
    $userAccount = @{}             # camelCase private variable
    $connectionString = "..."      # camelCase private variable
}
```

### Module and Script Naming

**Modules** should use PascalCase without verb:
```
UserManagement.psm1
NetworkUtilities.psm1
CompanyAutomation.psm1
```

**Scripts** should use Verb-Noun pattern:
```
Get-UserReport.ps1
...
1. **Functions/Cmdlets**: Use approved `Verb-Noun` pattern
2. **Parameters**: Follow PowerShell standard names
3. **Public identifiers**: Use PascalCase
4. **Private variables**: Optional camelCase for distinction
5. **Nouns**: Use singular form
6. **Be specific**: `$EmailAddress` not `$Email`, `$UserPrincipalName` not `$UPN`
7. **Consistency**: Match existing cmdlet patterns
...
## Function Structure & Cmdlet Design

### Always Use [CmdletBinding()]

**All functions should start with `[CmdletBinding()]`** - even simple ones. This unlocks crucial PowerShell features.

```powershell\n# Minimum function template\nfunction Get-Something {\n    [CmdletBinding()]\n    param()\n    \n    process {\n        # Function logic\n    }\n}\n```

**Benefits of CmdletBinding:**
- Enables `-Verbose` and `-Debug` common parameters
- Enables `-ErrorAction` and `-WarningAction` control
- Provides `$PSCmdlet` automatic variable
- Enables `ShouldProcess` for `-WhatIf` and `-Confirm`
- Makes function behave like a compiled cmdlet

### Explicit Block Structure: param(), begin, process, end

**Write functions in execution order** for clarity and maintainability:

```powershell
function Get-UserData {\n    [CmdletBinding()]\n    param(\n        [Parameter(ValueFromPipeline)]\n        [string]$UserName\n    )\n    \n    begin {\n        # Runs once before pipeline processing\n        Write-Verbose \"Initializing connection\"\n        $Results = @()\n    }\n    \n    process {\n        # Runs once for each pipeline input\n        Write-Verbose \"Processing: $UserName\"\n        $Results += Get-ADUser $UserName\n    }\n    \n    end {\n        # Runs once after all pipeline processing\n        Write-Verbose \"Processed $($Results.Count) users\"\n        return $Results\n    }\n}\n```

**Why explicit blocks?**
- Makes execution flow crystal clear\n- Easier to debug and maintain
- Proper pipeline processing\n- Avoids confusion about when code runs

**Note**: While PowerShell allows omitting the `end` keyword (treating unnamed block as `end`), and has a `filter` keyword (treating as `process`), **always use explicit names** for clarity.

### Use [OutputType()] Attribute

**Document what your function returns** with the `[OutputType()]` attribute:
...
**Benefits:**
- IntelliSense and tab completion support\n- Documentation for users\n- Design clarity for developers\n- Helps automated tools

### Pipeline Input Patterns

**Support pipeline input** for PowerShell-native workflows:

```powershell
function Set-UserProperty {\n    [CmdletBinding()]\n    param(\n        # Pipeline by value\n        [Parameter(Mandatory, ValueFromPipeline)]\n        [string]$UserName,\n        \n        # Pipeline by property name\n        [Parameter(ValueFromPipelineByPropertyName)]\n        [string]$Department\n    )\n    \n    process {\n        # Process each pipeline item\n        Write-Verbose \"Setting $UserName department to $Department\"\n    }\n}\n\n# Usage:\n\"john\", \"jane\" | Set-UserProperty -Department IT\n\nGet-ADUser -Filter * | Set-UserProperty  # Uses property names\n```

### Tool vs Controller Scripts

**Distinguish between reusable tools and specific controllers:**

#### Tools (Reusable Functions)
- Designed for maximum reusability
- Accept input ONLY via parameters
- Output raw, unformatted data to pipeline
- No direct screen output (use Write-Verbose/Debug instead)
- Typically in modules or function libraries
...
#### Controllers (Specific Automation)
- Designed for a specific business process\n- May call multiple tools/cmdlets\n- Can format output for screen or reports\n- Not meant to be reusable\n- Typically standalone scripts

```powershell
# Controller Script - Specific Purpose\n<#\n.SYNOPSIS\n    Daily disk space report for IT team\n#>\n[CmdletBinding()]\nparam()\n\n# Call reusable tool\n$diskData = Get-DiskInfo -ComputerName (Get-Content servers.txt)\n\n# Format for specific business need\n$report = $diskData | ForEach-Object {\n    [PSCustomObject]@{\n        Server     = $_.ComputerName\n        Drive      = $_.DeviceID\n        'Size GB'  = [math]::Round($_.SizeBytes / 1GB, 2)\n        'Free GB'  = [math]::Round($_.FreeBytes / 1GB, 2)\n        'Free %'   = [math]::Round(($_.FreeBytes / $_.SizeBytes) * 100, 1)\n    }\n}\n\n# Output formatted for humans\n$report | Format-Table -AutoSize\n$report | Export-Csv \"DiskReport_$(Get-Date -Format 'yyyyMMdd').csv\" -NoTypeInformation\n```

**Key Distinction:**
- **Tools output raw data** (bytes) → Reusable in any context
- **Controllers format data** (GB, %) → Specific business need

---

## Output Streams & Formatting

### Understanding PowerShell Output Streams

PowerShell has **6 output streams** with different purposes:

| Stream | Command | Purpose | Capturable | When to Use |
|--------|---------|---------|------------|-------------|
| **1. Success** | (default output) | Function results | Yes | Primary function output |
| **2. Error** | `Write-Error` | Error messages | Yes | Terminating/non-terminating errors |
| **3. Warning** | `Write-Warning` | Warning messages | Yes | Important alerts, not errors |
| **4. Verbose** | `Write-Verbose` | Detailed operation info | Yes | Implementation details for users |
| **5. Debug** | `Write-Debug` | Debug information | Yes | Troubleshooting for developers |
| **6. Information** | `Write-Information` | Informational messages | Yes | Structured output (PS 5.0+) |
| **N/A. Host** | `Write-Host` | Console display only | No (PS 5.x), Yes (PS 7+) | Formatting, interactive prompts |

### When to Use Each Stream

#### Success Stream (Default Output)
**Purpose**: Function results that flow through the pipeline

```powershell
function Get-UserAccount {\n    [CmdletBinding()]\n    param([string]$UserName)\n    \n    # This goes to success stream (pipeline)\n    Get-ADUser $UserName | Select-Object Name, Email\n}\n\n# Usage: output can be piped\nGet-UserAccount -UserName \"john\" | Export-Csv users.csv\n```

#### Write-Error
**Purpose**: Report errors without stopping execution

```powershell
function Get-UserAccount {\n    [CmdletBinding()]\n    param([string]$UserName)\n    \n    if (-not (Test-Connection $UserName -Quiet -Count 1)) {\n        Write-Error \"Cannot reach user $UserName\"\n        return\n    }\n    \n    Get-ADUser $UserName\n}\n```

#### Write-Warning  
**Purpose**: Important information that's not an error

```powershell
function Set-UserMailbox {\n    [CmdletBinding()]\n    param([string]$UserName)\n    \n    $user = Get-ADUser $UserName\n    if ($user.PasswordExpired) {\n        Write-Warning \"$UserName password has expired - mailbox access may be limited\"\n    }\n    \n    # Continue with mailbox operation\n}\n```

#### Write-Verbose
**Purpose**: Detailed progress for users running your script with `-Verbose`

```powershell
function Get-UserReport {\n    [CmdletBinding()]\n    param([string[]]$UserNames)\n    \n    begin {\n        Write-Verbose \"Starting user report generation\"\n    }\n    \n    process {\n        foreach ($user in $UserNames) {\n            Write-Verbose \"Processing user: $user\"\n            # Get user data\n        }\n    }\n    \n    end {\n        Write-Verbose \"Completed processing $($UserNames.Count) users\"\n    }\n}\n\n# User enables with -Verbose switch\nGet-UserReport -UserNames @(\"john\",\"jane\") -Verbose\n```

#### Write-Debug
**Purpose**: Debugging information for developers/maintainers

```powershell
function Get-UserAccount {\n    [CmdletBinding()]\n    param([string]$Filter)\n    \n    Write-Debug \"Filter value: $Filter\"\n    Write-Debug \"Entering AD query section\"\n    \n    $users = Get-ADUser -Filter $Filter\n    \n    Write-Debug \"Retrieved $($users.Count) users\"\n    return $users\n}\n\n# Developer enables with -Debug switch (prompts for breakpoint)\nGet-UserAccount -Filter \"Department -eq 'IT'\" -Debug\n\n# Or set preference to see without prompts\n$DebugPreference = \"Continue\"\nGet-UserAccount -Filter \"Department -eq 'IT'\"\n```

#### Write-Information
**Purpose**: Structured informational output (PowerShell 5.0+)

```powershell
function Get-ProcessData {\n    [CmdletBinding()]\n    param()\n    \n    Write-Information \"Starting process scan\" -Tags 'Process','Scan'\n    \n    $data = Get-Process | Where-Object CPU -gt 100\n    \n    Write-Information \"Found $($data.Count) high-CPU processes\" -Tags 'Process','Result'\n    \n    return $data\n}\n\n# Capture information stream\n$result = Get-ProcessData -InformationVariable info\n$info | Where-Object Tags -contains 'Result'\n```

#### Write-Host (Special Case)
**Purpose**: Direct console output for formatting or interactive scenarios

**PowerShell 5.x**: Output cannot be captured or redirected (use sparingly)  \n**PowerShell 7+**: Wraps Write-Information, can be captured (safer to use)

```powershell
function Show-Menu {\n    Write-Host \"==== Main Menu ====\" -ForegroundColor Cyan\n    Write-Host \"1. Option One\"\n    Write-Host \"2. Option Two\"\n    Write-Host \"3. Exit\"\n    Write-Host \"==================\" -ForegroundColor Cyan\n    \n    $choice = Read-Host \"Select an option\"\n    return $choice\n}\n\n# Also acceptable for formatted display functions\nfunction Format-ProgressReport {\n    param($Data)\n    \n    Write-Host \"Processing Complete\" -ForegroundColor Green\n    Write-Host \"  Success: $($Data.SuccessCount)\" -ForegroundColor Green\n    Write-Host \"  Failed: $($Data.FailCount)\" -ForegroundColor Red\n}\n```
...
```powershell
function Process-Users {\n    [CmdletBinding()]\n    param([string[]]$UserNames)\n    \n    $i = 0\n    $total = $UserNames.Count\n    \n    foreach ($user in $UserNames) {\n        $i++\n        $percentComplete = ($i / $total) * 100\n        \n        Write-Progress -Activity \"Processing Users\" `\n                       -Status \"Processing $user ($i of $total)\" `\n                       -PercentComplete $percentComplete\n        \n        # Do work\n        Start-Sleep -Milliseconds 500  # Simulated work\n    }\n    \n    Write-Progress -Activity \"Processing Users\" -Completed\n}\n```

### Output Stream Best Practices

1. **Success stream**: Function results only (pipeable data)
2. **Write-Error**: Errors that shouldn't stop execution\n3. **Write-Warning**: Important alerts that aren't errors\n4. **Write-Verbose**: Implementation details for `-Verbose` users\n5. **Write-Debug**: Debugging info for `-Debug` maintainers\n6. **Write-Information**: Structured messages with tags (PS 5+)
7. **Write-Host**: Only for formatting/display functions or interactive prompts
8. **Write-Progress**: Long-running operations with progress indicators

### Format Files for Custom Objects

**Don't use Format-* inside functions**. Instead, create `.format.ps1xml` files and reference in module manifest:

```powershell
# UserManagement.psd1 (module manifest)
@{\n    ModuleVersion = '1.0'\n    FormatsToProcess = @('UserManagement.Format.ps1xml')\n}\n\n# UserManagement.Format.ps1xml\n<Configuration>\n  <ViewDefinitions>\n    <View>\n      <Name>UserAccount</Name>\n      <ViewSelectedBy>\n        <TypeName>Company.UserAccount</TypeName>\n      </ViewSelectedBy>\n      <TableControl>\n        <TableHeaders>\n          <TableColumnHeader><Label>Name</Label></TableColumnHeader>\n          <TableColumnHeader><Label>Email</Label></TableColumnHeader>\n        </TableHeaders>\n        <TableRowEntries>\n          <TableRowEntry>\n            <TableColumnItems>\n              <TableColumnItem><PropertyName>Name</PropertyName></TableColumnItem>\n              <TableColumnItem><PropertyName>EmailAddress</PropertyName></TableColumnItem>\n            </TableColumnItems>\n          </TableRowEntry>\n        </TableRowEntries>\n      </TableControl>\n    </View>\n  </ViewDefinitions>\n</Configuration>\n\n# Function returns typed object\nfunction Get-UserAccount {\n    [PSCustomObject]@{\n        PSTypeName = 'Company.UserAccount'\n        Name       = 'John Doe'\n        EmailAddress = 'john@company.com'\n    }\n}\n```

### Only Output One Type of Object

**Functions should output a consistent object type:**

```powershell
# Bad - Mixed output types\nfunction Get-Data {\n    if ($SomeCondition) {\n        \"Error occurred\"  # String\n    } else {\n        [PSCustomObject]@{ Name = \"John\" }  # Object\n    }\n}\n\n# Good - Consistent output\nfunction Get-Data {\n    [CmdletBinding()]\n    [OutputType([PSCustomObject])]\n    param()\n    \n    if ($SomeCondition) {\n        Write-Error \"Error occurred\"  # Error stream\n        return\n    }\n    \n    # Always returns same type or nothing\n    [PSCustomObject]@{ Name = \"John\" }\n}\n```

**Exceptions:**
1. **Internal functions** can return multiple types (not exposed to users)
2. **Functions returning related types** from common base class (e.g., FileInfo/DirectoryInfo)
...
## Readability & Commenting Standards
...
#### Comment When:
- **Complex business logic**: Explain the reasoning behind non-obvious decisions
- **Workarounds**: Document why a hack or workaround exists
- **Performance considerations**: Explain optimizations that reduce clarity
- **External dependencies**: Document API quirks, version requirements
- **TODO/FIXME**: Mark areas needing improvement

```powershell
# Good - Explains WHY
# Set 30-second timeout because Azure Functions cold start can take 20+ seconds
$timeout = 30
...
#### Avoid Commenting When Code Can Be Self-Documenting:

```powershell
# Bad - Comment needed because of poor naming
$d = (Get-Date).AddDays(-7)  # Get date from 7 days ago
$u = Get-ADUser -Filter "whenCreated -gt '$d'"  # Get recent users

# Good - Names explain intent, no comment needed
$sevenDaysAgo = (Get-Date).AddDays(-7)
$recentUsers = Get-ADUser -Filter "whenCreated -gt '$sevenDaysAgo'"
```

### Comment Style and Placement

#### Single-Line Comments
Place above the code they describe, with a blank line before:

```powershell
# Check if module is already imported to avoid reconnection overhead
...
Use sparingly for parameter explanations:

```powershell
Get-ADUser -Filter * `
    -Properties whenCreated `
    -Server $dc  # Must query specific DC to avoid replication lag
```

#### Block Comments
For longer explanations, use proper block comment syntax:

```powershell
<#
    This function uses regex lookahead to validate email format while avoiding
    catastrophic backtracking. The pattern was chosen based on RFC 5322 section 3.4.1
    but simplified to avoid ReDoS vulnerabilities documented in CVE-2021-12345.
#>
...
# Bad
$u = Get-ADUser $n
$m = Get-Mailbox $u.UserPrincipalName
$m.MaxSendSize = 50MB

# Good
$user = Get-ADUser $userName
$mailbox = Get-Mailbox $user.UserPrincipalName
$mailbox.MaxSendSize = 50MB
```
...
# Good - Clear intent
function Convert-CsvToUserAccount {
...
```powershell
# Bad - Complex inline logic requires comments
if (($user.LastLogonDate -lt (Get-Date).AddDays(-90)) -and 
    ($user.PasswordLastSet -lt (Get-Date).AddDays(-365)) -and
    (-not $user.LockedOut)) {
    # User is stale but not locked
    Disable-ADAccount $user
}

# Good - Logic extracted to self-documenting function
if (Test-UserStale $user) {
    Disable-ADAccount $user
}

function Test-UserStale {
    param($User)
    $ninetyDaysAgo = (Get-Date).AddDays(-90)
    $oneYearAgo = (Get-Date).AddDays(-365)
    
    return ($User.LastLogonDate -lt $ninetyDaysAgo) -and
           ($User.PasswordLastSet -lt $oneYearAgo) -and
           (-not $User.LockedOut)
}
```
...
**All exported functions must have complete comment-based help**. This is not optional.
...
function Get-UserReport {
    ...
    .DESCRIPTION
        Retrieves user account information from Active Directory and Exchange Online,
        correlates last logon dates, mailbox usage, and license assignments.
        
        Supports bulk processing via CSV import and interactive single-user mode.
        Requires Global Reader or User Administrator role in Azure AD.
    
    .PARAMETER UserName
        User principal name or email address of the target user.
        Accepts pipeline input by value.
    
    .PARAMETER CsvPath
        Path to CSV file containing user list. CSV must have 'UserPrincipalName' column.
    
    .PARAMETER IncludeMailboxStats
        Include mailbox size, item count, and last access time in report.
        Warning: Significantly increases runtime for large user sets.
    
    .EXAMPLE
        Get-UserReport -UserName "john@company.com"
        
        Generates report for single user with default options.
    
    .EXAMPLE
        Get-UserReport -CsvPath "users.csv" -IncludeMailboxStats | Export-Csv report.csv
        
        Processes bulk user list from CSV and exports with mailbox statistics.
    
    .EXAMPLE
        "john@company.com", "jane@company.com" | Get-UserReport
        
        Accepts pipeline input for batch processing.
    ...
    .NOTES
        ...
        Required Permissions:
        - Azure AD: Global Reader or User Administrator
        - Exchange Online: View-Only Organization Management
        
        Setup Instructions:
        1. Install required modules: Install-Module Microsoft.Graph.Users, ExchangeOnlineManagement
        2. Connect to services: Connect-MgGraph -Scopes "User.Read.All"; Connect-ExchangeOnline
        3. Run script with appropriate parameters
    
    .LINK
        https://docs.microsoft.com/powershell/module/microsoft.graph.users
    #>
    [CmdletBinding()]
    param(
        [Parameter(Mandatory, ValueFromPipeline)]
...
#### Comment-Based Help Best Practices:

1. **SYNOPSIS**: Single-line summary (under 80 characters)
2. **DESCRIPTION**: Full explanation including:
   - What the function does
   - Key features and limitations
   - Authentication/permission requirements
   - Important warnings
3. **PARAMETER**: Document EVERY parameter with:
   - Clear description
   - Expected format/values
   - Default behavior if omitted
   - Pipeline input if applicable
4. **EXAMPLE**: Provide 3-5 examples showing:
   - Basic usage
   - Common scenarios
   - Advanced options
   - Pipeline usage
...
5. **NOTES**: Include:
   ...
   - Required permissions
   - Setup instructions
6. **LINK**: Related documentation URLs

### Inline Documentation Limits

**Don't over-comment**. Too many comments indicate code that's too complex.
...
foreach ($user in $users) {
    # Check last logon
    if ($user.LastLogonDate -lt $thirtyDaysAgo) {
        # Disable user
        Disable-ADAccount $user
    }
}

# Good - Only essential comments
$thirtyDaysAgo = (Get-Date).AddDays(-30)
$inactiveUsers = Get-ADUser -Filter * | Where-Object {
    $_.LastLogonDate -lt $thirtyDaysAgo
}

# Disable accounts inactive for 30+ days per IT-Policy-2024-03
$inactiveUsers | Disable-ADAccount
```

### TODO and FIXME Comments

Use standardized tags for marking work in progress:

```powershell
# TODO: Add retry logic for transient network errors (Issue #1234)

# FIXME: This timeout is too short for large mailboxes - causes false failures
$timeout = 30

# HACK: Graph API beta endpoint required due to bug in v1.0 (MS Support Case 98765)
$uri = "https://graph.microsoft.com/beta/users"

# OPTIMIZE: Consider caching AD group membership to reduce LDAP queries
$groups = Get-ADPrincipalGroupMembership $user
```

**Format**: `# TAG: Description (Reference)`
- **TODO**: Feature/improvement to add
- **FIXME**: Known bug to fix
- **HACK**: Workaround for external issue (document why)
- **OPTIMIZE**: Performance improvement opportunity
- **NOTE**: Important clarification
- **WARNING**: Potential issue for maintainers

Always include ticket numbers, dates, or references for context.
...
```powershell
# Bad - Comment no longer accurate after code change
# Connects to Exchange Online
Connect-MgGraph -Scopes "User.Read.All"

# Good - Updated comment or removed
Connect-MgGraph -Scopes "User.Read.All"
```
...
### Regional Comments for Large Functions

For large functions (100+ lines), use regional comments for navigation:
...
function Process-ComplexWorkflow {
    ...
    begin {
        ...
        Write-Verbose "Initializing connections"
        $results = @()
        Connect-Services
...
**Note**: Many editors support code folding on `#region`/`#endregion`, making them useful for large scripts. Use sparingly - if you need many regions, consider breaking into multiple functions instead.
...
Describe "Test-EmailComprehensive" {
    Context "Format Validation" {
        It "Should accept valid email addresses" {
            $result = Test-EmailComprehensive -EmailAddress "user@company.com"
    ...
    Context "Domain Validation" {
        It "Should accept emails from required domain" {
            $result = Test-EmailComprehensive -EmailAddress "user@company.com" -RequiredDomain "company.com"
            $result.DomainValid | Should -Be $true
        }
        
        It "Should reject emails from other domains" {
            $result = Test-EmailComprehensive -EmailAddress "user@external.com" -RequiredDomain "company.com"
...
Describe "Script End-to-End" {
    It "Should process CSV input successfully" {
        # Create test CSV
        $testData = @(
            @{EmailAddress = "test1@company.com"},
...
function New-FakeUsers {
    ...
    for ($i=1; $i -le $Count; $i++) {
        $list.Add([pscustomobject]@{Email="user$i@company.com"; Dept='IT'; Enabled=$true})
    }
    return $list
}
```
...
$cert = New-SelfSignedCertificate -Type CodeSigningCert -Subject "CN=Company Dev Code Signing" -CertStoreLocation Cert:\CurrentUser\My
```

### Signing a Script
```powershell
$cert = Get-ChildItem Cert:\CurrentUser\My | Where-Object { $_.Subject -like '*Company Dev Code Signing*' }
...
.NOTES
    Author: Company Automation Team (with AI assistance)
    Version: 1.0
    Created: 2025-11-19
...
### v2.2.0 (2025-11-22)
Enhancement update integrating PoshCode community best practices for comprehensive coding standards.
- Added: Code Formatting & Style section (OTBS, 4-space indentation, 115-character line limit, PascalCase/lowercase conventions, operators spacing, pipeline alignment)
- Added: Naming Conventions section (Verb-Noun pattern, approved verbs reference, PascalCase parameters, boolean naming, acronym handling)
- Added: Function Structure & Cmdlet Design section ([CmdletBinding()] mandatory usage, explicit begin/process/end blocks, [OutputType()] attribute, pipeline input patterns, Tool vs Controller distinction)
- Added: Output Streams & Formatting section (6 stream types with usage guidance, Write-Error/Warning/Verbose/Debug/Information/Host distinctions, Write-Progress for long operations, format files best practices, single output type rule)
- Added: Readability & Commenting Standards section (when to comment vs self-documenting code, comment style and placement, TODO/FIXME/HACK tags, comment-based help requirements, regional comments for large functions)
- Improved: Table of Contents updated with new sections in Core Framework category
- Improved: Comprehensive code examples throughout new sections demonstrating proper and improper patterns
- Clarified: PowerShell 7+ Write-Host behavior (wraps Write-Information, capturable) vs PowerShell 5.x (uncapturable)
- Standardized: Alignment of internal patterns with broader PowerShell community standards from PoshCode guide
...
This guide integrates content and best practices from the following sources:

#### PowerShell Practice and Style Guide
**Source**: [https://poshcode.gitbook.io/powershell-practice-and-style](https://poshcode.gitbook.io/powershell-practice-and-style)  
**Maintained by**: PoshCode Community  
**License**: Creative Commons  

The following sections incorporate guidance from the PoshCode PowerShell Practice and Style Guide:
- Code Formatting & Style (OTBS, indentation, line length, capitalization)
- Naming Conventions (Verb-Noun pattern, approved verbs, PascalCase standards)
- Function Structure & Cmdlet Design ([CmdletBinding()], explicit blocks, pipeline patterns)
- Output Streams & Formatting (stream types, usage guidelines, format files)
- Readability & Commenting Standards (when to comment, comment-based help, TODO/FIXME conventions)

These community standards have been adapted and extended with enterprise patterns including:
- Triple input pattern (Array/CSV/Interactive parameter sets)
- Comprehensive validation framework with skip switches
- Single connection pattern for service authentication
- Write-Status wrapper function for structured output
- Security-first development rules
- PSScriptAnalyzer mandatory validation

#### Microsoft Documentation
**Source**: [https://docs.microsoft.com/powershell](https://docs.microsoft.com/powershell)  
**Publisher**: Microsoft Corporation

Official PowerShell documentation referenced for:
- Module requirements and compatibility
- API integration patterns (Microsoft Graph, Exchange Online)
- Cross-platform PowerShell considerations
- Security features (Constrained Language Mode, JEA, WDAC)

#### Additional References
- **Pester Documentation**: [https://pester.dev](https://pester.dev) - Testing framework guidance
- **PSScriptAnalyzer**: [https://github.com/PowerShell/PSScriptAnalyzer](https://github.com/PowerShell/PSScriptAnalyzer) - Static code analysis
- **PowerShell Gallery**: [https://www.powershellgallery.com](https://www.powershellgallery.com) - Module ecosystem

### Enterprise-Specific Contributions

The following patterns and frameworks are original contributions developed through enterprise PowerShell automation:
- Multi-layer validation architecture (Format → Business Logic → System → Pre-execution)
- Hybrid authentication strategy for multi-service scripts
- Result object pattern with success tracking and CSV export
- NinjaOne RMM deployment patterns
- H700 system integration patterns
- Acumatica ERP automation patterns

### License & Usage

This guide is maintained for internal use and shared publicly for community benefit. When incorporating content from external sources, appropriate attribution has been provided. Users of this guide should respect the licenses of referenced materials.

**Recommended Citation**:
```
PowerShell Script Development Best Practices Guide (v2.2.0).
Incorporates content from PoshCode PowerShell Practice and Style Guide (CC License).
```
